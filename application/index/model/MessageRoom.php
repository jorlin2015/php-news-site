...
class MessageRoom extends Model{
	...
	private const MESSAGESQLCONT = "select count(m.rmsg_id) sum from t_message_room m, t_user u where m.rmsg_room = :id and m.rmsg_status = 1 and u.user_status = 1 and m.rmsg_sender=u.user_id";
	private const MESSAGESQLPAGE = "select m.rmsg_type type, m.rmsg_content content, m.rmsg_time time, u.user_id id, u.user_alias name, u.user_img img from t_message_room m, t_user u where m.rmsg_room = :id and m.rmsg_status = 1 and u.user_status = 1 and m.rmsg_sender=u.user_id order by m.rmsg_time asc limit :start,:size";
	...
	public static function getMessagePage($room, $page, $size){
		$start = ($page - 1) * $size;
		return Db::query(self::MESSAGESQLPAGE, ["id" => $room, "start" => $start, "size" => $size]);
	}
