...
class MessagePrivate extends Model{
	...
	private const MESSAGESQLPAGE = "select m.pmsg_type type, m.pmsg_content content, m.pmsg_time time,u.user_id id, u.user_alias name, u.user_img img from t_message_private m, t_user u where m.pmsg_status = 1 and (m.pmsg_sender = :sender and m.pmsg_receiver = :receiver or m.pmsg_sender = :sender1 and m.pmsg_receiver = :receiver1) and m.pmsg_sender = u.user_id order by m.pmsg_time asc limit :start,:end";
	...
	public static function getMessagePage($sender, $receiver, $page, $size){
		$start = ($page - 1) * $size;
		$end =  $start + $size - 1;
		return Db::query(self::MESSAGESQLPAGE, ["sender" => $sender, "receiver"=>$receiver, "sender1" => $receiver, "receiver1" => $sender, "start" => $start, "end" => $end]);
	}
